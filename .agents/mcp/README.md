...
| `project_state` | `plan/`, `discovery/` or `architecture/` exists | Summarises active plans, discoveries and ADRs with their status lines |
...
nothing. Adding a `plan/` directory to a KCL library is all it takes for
`project_state` to appear there.
